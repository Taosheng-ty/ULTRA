...
class Dla_accept_prob(BasicAlgorithm):
    ...
    * Qingyao Ai, Keping Bi, Cheng Luo, Jiafeng Guo, W. Bruce Croft. 2018. Unbiased Learning to Rank with Unbiased Propensity Estimation. In Proceedings of SIGIR '18
    ...
    def __init__(self, data_set, exp_settings, forward_only=False):
        ...
        self.hparams = tf.contrib.training.HParams(
            ...
            loss_func='click_weighted_prob_cross_entropy',            # Select Loss function
        ...
        print(self.output.get_shape(),"self.output.get_shape()",\
              len(self.docid_inputs),self.docid_inputs[0].get_shape(),"self.docid_inputs[0].get_shape()")
...
#         train_output = self.output[:,:self.rank_list_size]
        ...
        if not forward_only:
            ...
            elif self.hparams.loss_func == 'click_weighted_prob_cross_entropy':
                self.loss_func = self.click_weighted_prob_cross_entropy_loss
            ...
            self.propensity_weights = self.get_normalized_weights(self.propensity)
            ...
            self.relevance_weights = self.get_normalized_weights(train_output)
...
#                         int((list_size+1)/4) + 1,
            ...
            output=self.logits_to_prob(tf.concat(output_propensity_list,1))
        return output
...
#             print( outputs[2],"out from sess run")
    ...
    def click_weighted_prob_cross_entropy_loss(self, output, labels, propensity_weights, name=None):
        """Computes listwise softmax loss with propensity weighting.
        ...
        """
        loss = None
        with tf.name_scope(name, "click_weighted_prob_cross_entropy_loss",[output]):
            label_dis = labels*propensity_weights / tf.reduce_sum(labels*propensity_weights, 1, keep_dims=True)
            loss = -tf.reduce_sum(tf.math.log(output)*label_dis,1) * tf.reduce_sum(labels*propensity_weights, 1)
        return tf.reduce_sum(loss) / tf.reduce_sum(labels*propensity_weights)
