...
class DirectLabelFeed_with_clicks(BasicInputFeed):
    ...
    def __init__(self, model, batch_size, hparam_str, session=None):
        ...
        print('Create direct label feed with clicks')
#         print(hparam_str)
#         self.hparams.parse(hparam_str)
        ...
        self.start_index = 0
        self.count = 1
        self.click_list_size = model.rank_list_size
        self.rank_list_size = model.max_candidate_num
        self.feature_size = model.feature_size
        self.batch_size = batch_size
        self.model = model
        
    def prepare_true_labels_with_index(self, data_set, index, docid_inputs, letor_features, labels,clicks,types,\
                                       check_validation=True):
        i = index
        # Generate label list.
        label_list = [0 if data_set.initial_list[i][x] < 0 else data_set.labels[i][x] for x in range(self.click_list_size)]
        click_list = None
        if self.hparams.oracle_mode:
            ...
            while check_validation and sum(click_list) == 0 and sample_count < self.MAX_SAMPLE_ROUND_NUM:
                click_list, _, types_list = self.click_model.sampleClicksForOneList(list(label_list))
                sample_count += 1
#         print(sample_count,"sample_count",label_list,"label_list",click_list,"click_lisrt")
        # Check if data is valid
        if check_validation and sum(click_list)== 0==0:
            return
#         if  sum(click_list)== 0:
# #             print(label_list,"label_list",click_list,"click_lisrt")
#             return
    ...
    def get_batch(self, data_set, check_validation=False):
        ...
        docid_inputs, letor_features, labels,click = [], [], [],[]
        rank_list_idxs = []
        types=[]
        for _ in range(self.batch_size):
            ...
            self.prepare_true_labels_with_index(data_set, i,
                                docid_inputs, letor_features, labels,click,types, check_validation)
        ...
        for length_idx in range(self.rank_list_size):
            ...
            batch_labels.append(
                np.array([labels[batch_idx][length_idx]
                        for batch_idx in range(local_batch_size)], dtype=np.float32))
            batch_types.append(np.array([types[batch_idx][length_idx]
                        for batch_idx in range(local_batch_size)], dtype=np.float32))
        ...
        if hasattr(self.model,"clicks"):
            for length_idx in range(self.click_list_size):
                batch_clicks.append(
                    np.array([clicks[batch_idx][length_idx]
                            for batch_idx in range(local_batch_size)], dtype=np.float32))
        if hasattr(self.model,"types"):
            for length_idx in range(self.click_list_size):
                batch_types.append(
                    np.array([types[batch_idx][length_idx]
                            for batch_idx in range(local_batch_size)], dtype=np.float32))
    ...
    def get_next_batch(self, index, data_set, check_validation=False):
        ...
        docid_inputs, letor_features,labels,clicks,types = [], [], [],[],[]
        
        num_remain_data = len(data_set.initial_list) - index
        for offset in range(min(self.batch_size, num_remain_data)):
            i = index + offset
            self.prepare_true_labels_with_index(data_set, i, docid_inputs, letor_features, labels,clicks,types, check_validation)
#         print(clicks,"clicks")
#         print(labels,"labels")
        local_batch_size = len(docid_inputs)
        letor_features_length = len(letor_features)
...
#         print(batch_clicks)
        for l in range(self.rank_list_size):
            ...
            input_feed[self.model.labels[l].name] = batch_labels[l]
        if hasattr(self.model,"clicks"):
            for l in range(self.click_list_size):
                input_feed[self.model.clicks[l].name] = batch_clicks[l]
        if hasattr(self.model,"types"):
            for l in range(self.click_list_size):
