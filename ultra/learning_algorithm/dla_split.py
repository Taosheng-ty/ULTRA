...
class DLA_split(BaseAlgorithm):
    ...
    * Qingyao Ai, Keping Bi, Cheng Luo, Jiafeng Guo, W. Bruce Croft. 2018. Unbiased Learning to Rank with Unbiased Propensity Estimation. In Proceedings of SIGIR '18
    ...
    def __init__(self, data_set, exp_settings, forward_only=False):
        ...
        """
        print('Building DLA_split')
        super(DLA_split, self).__init__(data_set, exp_settings, forward_only)
        ...
        pkl_file = open(self.hparams.kmeans_file, 'rb')
        self.kmeans_dict = pickle.load(pkl_file)
        pkl_file.close()
        ...
        self.select_ind=tf.placeholder(tf.int64, shape=[None],
                                            name="select")
        ...
        self.output=tf.reshape(self.output,(-1,self.max_candidate_num))
        print(self.output,"self.output.shape after")
        self.output = tf.nn.embedding_lookup(self.output, self.select_ind)
        ...
        train_output=tf.reshape(train_output,(-1,self.rank_list_size))
        train_output = tf.nn.embedding_lookup(train_output, self.select_ind)
...
#                         int((list_size+1)/4) + 1,
    ...
    def norm_post(self,x,means=1,stds=1,axis=0):
        x=(x-means)/(stds+1e-10)
        return x
    def find_cluster(self,input_X,param_dict):
        means=param_dict["means"]
        stds=param_dict["stds"]
        vh=param_dict["vh"]
        ind_cut=20
        kmeans=param_dict["kmeans"]
        input_X=self.norm_post(input_X,means,stds)
        input_X_proj=input_X@vh.T
        cluster_id=kmeans.predict(input_X_proj[:,:ind_cut])
        return cluster_id
    def step(self, session, input_feed, forward_only):
        ...
        ind0=input_feed[self.docid_inputs[0].name].astype(int)
        features=input_feed[self.letor_features.name][ind0]
        
        ind_select=self.find_cluster(features,self.kmeans_dict)
     #   ind_cut =self.kmeans_dict.ind_cut 
        ind_cut =20  
        ind_step=np.arange(0,features.shape[0])*ind_cut
        ind_select=ind_select+ind_step
        input_feed[self.select_ind.name]=ind_select
      #  print(ind_select,ind_step,"ind_select,ind_step")
        if not forward_only:
...
#                            self.click_metrics,
          ...
          #  print(features.shape,features[:1,:30],features[:1,-30:],features[:,0],features[:,-2],"features.shape,features[:2,:],features[:,0],features[:,-1]")
